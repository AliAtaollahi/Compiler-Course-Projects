...
printStatement returns [PrintStmt printRet] :
    PRINT LPAREN expression { $printRet = new PrintStmt($expression.expressionRet); $printRet.setLine($PRINT.getLine()); } RPAREN SEMICOLON;
...
assignStatement returns [AssignStmt assignStmtRet] : {$assignStmtRet = new AssignStmt(null,null);}
     ((i2=identifier LBRACK ex1=expression RBRACK{ArrayIdentifier arrayRet = new ArrayIdentifier($i2.identRet.getName(),$ex1.expressionRet);arrayRet.setLine($i2.identRet.getLine());$assignStmtRet.setLValue(arrayRet);}) | i1=identifier {$assignStmtRet.setLValue($i1.identRet);}) assignType = assign {$assignStmtRet.setBinaryOperator($assignType.op);} ex2 = expression {$assignStmtRet.setRValue($ex2.expressionRet);} SEMICOLON{$assignStmtRet.setLine($SEMICOLON.line);};
...
assign returns [BinaryOperator op]:
    (ASSIGN {$op = BinaryOperator.ASSIGN;} | 
    MUL_ASSIGN {$op = BinaryOperator.MUL_ASSIGN;} | 
    ADD_ASSIGN {$op = BinaryOperator.ADD_ASSIGN;} | 
    SUB_ASSIGN {$op = BinaryOperator.SUB_ASSIGN;} | 
    DIV_ASSIGN {$op = BinaryOperator.DIV_ASSIGN;} | 
    MOD_ASSIGN {$op = BinaryOperator.MOD_ASSIGN;});
